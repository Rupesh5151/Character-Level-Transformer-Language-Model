# -*- coding: utf-8 -*-
"""
  MINI LLM — Flask Web App Backend
  Author : ADARSH DWIVEDI
  Serves the training + generation API for the browser UI.
"""
...
# ──────────────────────────────────────────────
state = {
    "status"      : "idle",        # idle | loading | training | done | error
    "step"        : 0,
    "max_iters"   : 5000,
    "train_loss"  : [],
    "val_loss"    : [],
...
# Model artifacts (module-level so threads share them)
model_ref   = {"model": None}
enc_ref     = {"stoi": None, "itos": None,
               "train_data": None, "val_data": None}
lock        = threading.Lock()
stop_flag   = threading.Event()

# ──────────────────────────────────────────────
#  HYPER-PARAMS (overridable via API)
# ──────────────────────────────────────────────
DEFAULT_HP = dict(
    batch_size  = 32,
    block_size  = 64,
    ...
    subset_rows = 1000,
    temperature = 0.8,
    embed_dim   = 128,   # hidden size for the small transformer
)
...
# ──────────────────────────────────────────────
#  MODEL — Small Transformer (GPT-lite)
# ──────────────────────────────────────────────
...
def train_worker(hp: dict):
    ...
    try:
        ...
        # --- Training loop ---
        eval_every = max(50, hp["max_iters"] // 100)   # ~100 data points on chart
